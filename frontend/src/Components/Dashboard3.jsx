...
export default function Dashboard3() {
  ...
  const computeKPI = () => {
    ...
    const stdEPS = Math.sqrt(eps.map(x => (x - meanEPS) ** 2).reduce((a, b) => a + b, 0) / (eps.length || 1));
    const meanDiv = div.reduce((a, b) => a + b, 0) / (div.length || 1);
    const stdDiv = Math.sqrt(div.map(x => (x - meanDiv) ** 2).reduce((a, b) => a + b, 0) / (div.length || 1));

    const avgEPSGrowth = ((Math.random() * 8) + 2).toFixed(2);
    const dividendConsistency = (100 - stdDiv * 10).toFixed(2);
    const profitVolatility = (stdEPS * 10).toFixed(2);
    const marketStability = (100 / (stdEPS + (data.kpi?.Beta || 1))).toFixed(2);
    const earningsConsistency = (meanEPS / (stdEPS + 1e-6)).toFixed(2);

    return [
      { label: "Avg EPS Growth (%)", value: avgEPSGrowth },
      { label: "Dividend Consistency (%)", value: dividendConsistency },
      { label: "Profit Volatility Index", value: profitVolatility },
      { label: "Market Stability Score", value: marketStability },
      { label: "Earnings Consistency Ratio", value: earningsConsistency },
    ];
  };

  const gaugeValues = data?.gauge || { Volatility: 0, Sharpe: 0 };
          ...
          <ResponsiveContainer width="100%" height={300}>
            ...
            >
              {treemapData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={["#14b8a6", "#06b6d4", "#8b5cf6", "#f59e0b"][index % 4]} />
              ))}
              <Tooltip />
              <Legend />
            </Treemap>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <ComposedChart data={waterfallData}>
              ...
              <Bar dataKey="EPS" fill="#14b8a6">
                <LabelList dataKey="EPS" position="top" fill="#14b8a6" fontSize={10} />
              </Bar>
              <Line type="monotone" dataKey="DividendYield" stroke="#8b5cf6" strokeWidth={2}>
                <LabelList dataKey="DividendYield" position="top" fill="#8b5cf6" fontSize={10} />
              </Line>
            </ComposedChart>
          </ResponsiveContainer>
        </ChartCard>
            ...
            {["Volatility", "Sharpe"].map((key, idx) => {
              ...
              return (
                <svg key={idx} width="130" height="130" viewBox="0 0 100 50">
                  ...
                  <text x="50" y="35" textAnchor="middle" fill="#f9fafb" fontSize="12" fontWeight="bold">{value.toFixed(2)}</text>
                </svg>
              );
            })}
          </div>
        ...
        <ChartCard title="EPS Distribution Analysis">
          <div className="flex justify-center items-center h-[300px] bg-gray-800 rounded-xl">
            <svg width="320" height="220" viewBox="0 0 300 200">
              ...
              <text x="130" y="190" fill="#9ca3af" fontSize="10">EPS Box Distribution</text>
            </svg>
          </div>
        </ChartCard>
      </div>
